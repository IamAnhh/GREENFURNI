...
export class AccountcustomerService {
  ...
  checkMailExist(Mail: string): Observable<any> {
    ...
    return this._http
      .get<any>(this.apiUrl + '/accounts/' + Mail, requestOptions)
      .pipe(
  ...
  data$ = this.dataSource.asObservable(); // Dữ liệu sẽ được công khai dưới dạng Observable

  // Phương thức để cập nhật dữ liệu
  updateData(data: string) {
    this.dataSource.next(data); // Cập nhật dữ liệu mới
  }
  ...
  getAccountInfo(accountId: string): Observable<any> {
    ...
    return this._http
      .get<any>(`${this.apiUrl}/my-account/${accountId}`, requestOptions)
  ...
  updateAccountInfo(accountId: string, updatedInfo: any): Observable<any> {
    ...
    return this._http
      .put<any>(
        `${this.apiUrl}/my-account/${accountId}`,
        updatedInfo,
        requestOptions
      )
      .pipe(catchError(this.handleError));
  ...
  setDefaultAddress(accountId: string, index: number): Observable<any> {
    ...
    return this._http.put(url, {}).pipe(catchError(this.handleError));
  }
}
